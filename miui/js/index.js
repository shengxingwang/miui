	...
	var effect = function(){
		...
		for(var i = 0,len = items.length;i<len;i++){
			...
			items[i].onmouseover=function(){
				...
				//alert(dots.length);
				for(j = 0,jlen = dots.length;j<jlen;j++){
					(function(j){
						dots[j].onclick = function(){
							for(var x = 0,xlen =dots.length;x<xlen;x++){
								dots[x].className="cdot";
							}
							currenList.count=j;
							this.className="cdot active";
							animate(currenList,{marginLeft:-j*296},1,4)
						}
					})(j);
				}
			}
			...
			rbtn[i].onclick=function(){
				if(flag){
					flag = false;
					mlist[this.index].count++;
					if(mlist[this.index].count>=3){
						mlist[this.index].count=3;
					}
					for(var j = 0,jlen=control[this.index].children.length;j<jlen;j++){
						control[this.index].children[j].className="cdot";
					}
					control[this.index].children[mlist[this.index].count].className="cdot active";
					animate(mlist[this.index],{marginLeft:-mlist[this.index].count*296},1,4,function(){
						flag = true;
					});
				}
			}
			lbtn[i].onclick=function(){
				if(flag){
					flag = false;
					mlist[this.index].count--;
					if(mlist[this.index].count<=0){
						mlist[this.index].count=0;
					}
					for(var j = 0,jlen=control[this.index].children.length;j<jlen;j++){
						control[this.index].children[j].className="cdot";
					}
